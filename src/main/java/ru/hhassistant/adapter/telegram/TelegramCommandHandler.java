...
/**
 * Обрабатывает входящие Telegram updates: команды и callback-кнопки.
 *
 * <p>Тонкий адаптер — переводит Telegram events в вызовы портов/репозиториев.
 * Никакой бизнес-логики поиска здесь нет.
 *
 * <p>Состояние wizard-диалога хранится в памяти (ConcurrentHashMap).
 * При рестарте приложения состояние сбрасывается — пользователь повторяет /start.
 * Для продакшн-надёжности можно вынести в Redis/БД.
 */
@ApplicationScoped
public class TelegramCommandHandler {

    private static final Logger log = Logger.getLogger(TelegramCommandHandler.class);

    // Состояния конечного автомата Telegram-диалога
    enum State {
    ...
    /** Временные данные диалога (email, login_info, pending OTP). */
    ...
    @Inject HhAuthenticatedWebClient hhWebClient;
    @Inject TelegramOutboundClient outbound;
    @Inject TelegramMessageFormatter formatter;

    /**
     * Точка входа для всех Updates от бота.
     * Вызывается {@link TelegramBotAdapter} в long-polling loop.
     */
    public void handleUpdate(Update update) {
        try {
            ...
            log.errorf(ex, "telegram.handler_error updateId=%d", update.updateId());
        }
    }
    ...
    private void handleMessage(Message msg) {
        ...
        switch (state) {
            ...
            default -> {
                // В других состояниях текстовые сообщения игнорируются
            }
        }
    }
    ...
    private void handleLetterInput(long chatId, String text) {
        ...
        if (error != null) {
            outbound.sendHtml(chatId, "⚠️ " + error); return;
        }
    ...
    private void handleEmailInput(long chatId, String email) {
        if (!email.contains("@")) {
            outbound.sendHtml(chatId, "⚠️ Похоже, это не email. Попробуйте ещё раз."); return;
        }
        outbound.sendHtml(chatId, "⏳ Проверяю аккаунт...");
        var init = hhWebClient.initiateLogin(email);
        if (init.isError()) {
            outbound.sendHtml(chatId, "❌ " + init.errorMessage()); return;
        }
    ...
    private void handleOtpInput(long chatId, String code) {
        ...
        if (email == null || init == null) { handleStart(chatId); return; }

        outbound.sendHtml(chatId, "⏳ Проверяю код...");
        var result = hhWebClient.completeOtpLogin(email, code, init);
        if (!result.success()) {
            outbound.sendHtml(chatId, "❌ Неверный код. Попробуйте ещё раз:"); return;
        }
    ...
    private void handlePasswordInput(long chatId, String password) {
        ...
        if (!result.success()) {
            outbound.sendHtml(chatId, "❌ Неверный пароль. Попробуйте ещё раз:"); return;
        }
        finishAuth(chatId, email, result.hhtoken());
    }
    ...
    private void finishAuth(long chatId, String email, String hhtoken) {
        ...
        if (resumes.isEmpty()) {
            outbound.sendHtml(chatId, "❌ Нет резюме на аккаунте. Создайте резюме на hh.ru и нажмите /start."); return;
        }
        if (resumes.size() == 1) {
            var r = resumes.get(0);
    ...
    private void handleStartSearch(long chatId, int messageId) {
        ...
        if (settings == null || !isComplete(settings)) {
            outbound.sendHtml(chatId, "⚠️ Настройки неполные. Нажмите /start для настройки."); return;
        }
        ...
        outbound.editMessage(chatId, messageId,
            ...
            stopKeyboard());
        log.infof("search.started chatId=%d", chatId);
    }
    ...
    private void handleStopSearch(long chatId, int messageId) {
        ...
        log.infof("search.stopped chatId=%d", chatId);
    }
